...
* **image-url**: url of the image to VA scan - required if no image pipeline resource provided to this task (default to empty string)
* **image-digest**: SHA id of the image to VA scan - required if no image pipeline resource provided to this task (default to empty string)
...
* **scan-report-file**: filename for the scan report (json format) of the given image. It will be copied in the workspace (default to empty string)
...
* **scan-step-image**: image to use for the scan step (default to `icr.io/continuous-delivery/pipeline/pipeline-base-ubi:3.79`)
...
* **container-registry-apikey-secret-key**: field in the secret that contains the api key used to login to ibmcloud container registry (default to `apikey`)
* **resource-group**: target resource group (name or id) for the ibmcloud login operation (default to empty string)
* **image-url**: url of the image to build required if no image pipeline resource provided or no registry region, namespace and image name parameters are provided to this task (default to empty string)
* **registry-region**: container registry region id. required if no image-url or no image pipeline resources provided (default to empty string)
* **registry-namespace**: container registry namespace. required if no image-url or no image pipeline resources provided (default to empty string)
* **registry-create-namespace**: create container registry namespace if it doesn't already exists (default to `true`)
* **image-name**: image name. required if no image-url or no image pipeline resources provided (default to empty string)
* **path-to-context**: the path to the context that is used for the build (`.` meaning current directory) (default to `.`)
...
* **build-args**: build argument list in the format 'KEY=VALUE' with a key-value pair per line. (default to empty string)
...
* **check-step-image**: image to use for the check (pre-build) step (default to `icr.io/continuous-delivery/pipeline/pipeline-base-ubi:3.72`)
* **process-log-step-image**: image to use for the process log (post-build) steps (default to `icr.io/continuous-delivery/pipeline/pipeline-base-ubi:3.72`)
* **additional-tags**: comma-separated list of additional-tags (default to empty string)
* **additional-tags-script**: Shell script that allows to add tags for the image to be build. (default to empty string)
...
* **pipeline-debug**: Pipeline debug mode. Value can be 0 or 1. (default to `0`)
* **disable-caching**: Disable caching layers when building images. Value can be 0 or 1. (default to `0`)
...
* **check-and-build-step-image**: image to use for the scan step (default to `icr.io/continuous-delivery/pipeline/pipeline-base-ubi:3.72`)
* **pipeline-debug**: Pipeline debug mode. Value can be 0 or 1. (default to `0`)
...
* **resource-group**: target resource group (name or id) for the ibmcloud login operation (default to empty string)
* **cluster-region**: the ibmcloud region hosting the cluster (if value is `` it will default to the toolchain region) (default to empty string)
* **cluster-name**: name of the docker build cluster - required if no cluster pipeline resource provided to this task (default to empty string)
* **cluster-namespace**: the kubernetes cluster namespace where the docker engine is hosted/deployed (default to `build`)
* **registry-create-namespace**: create container registry namespace if it doesn't already exists (default to `true`)
* **image-url**: url of the image to build - required if no image pipeline resource provided to this task (default to empty string)
...
`)
* **check-step-image**: image to use for the check (pre-build) step (default to `icr.io/continuous-delivery/pipeline/pipeline-base-ubi:3.72`)
* **cluster-setup-step-image**: image to use for the cluster setup step (default to `icr.io/continuous-delivery/pipeline/pipeline-base-ubi:3.72`)
...
This task runs `docker` commands (build, inspect...) against a Docker engine running as a sidecar container, and pushes the resulting image to the [IBM Cloud Container Registry](https://cloud.ibm.com/docs/services/Registry?topic=registry-getting-started).
